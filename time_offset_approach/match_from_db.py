...
def identify_song(query_audio, min_vote_threshold=100, ratio_threshold=2.5):
    ...
    return best_song, best_votes, "MATCHED!"


if __name__ == "__main__":
    print("Starting identification...", flush=True)
    audio_path = "../chromaprint_approach/evaluation/Killa Klassic/concertKillaKlassic.mp3"

    song, votes, status = identify_song(audio_path)
    print(f"Result: {song}, {votes}, {status}", flush=True)
...
def recognize(query_audio):
    ...
    # threshold logic: 
    # if best_score < 0.40:
    #     return None, round(best_score ,3) , "low_confidence"
    # # if there is less difference then system is unsure
    # if (best_score - second_score) < 0.01: 
    #      # diff of best and second_best score
    #     return None, round(best_score, 3), "ambigous"

    return best_song, round(best_score, 3)

if __name__ == "__main__":

    song, score, reason = recognize("evaluation\Lafangey Parindey\query2LafangeyFull.m4a")
